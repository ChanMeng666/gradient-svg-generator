...
import {
  ...
  createSocialBadge,
  createRepoCard,
} from '@/utils/gradientGenerators/githubProfileGradients';
...
function defaultParams(overrides: Params = {}): Required<Params> {
  return { text: 'Test', colors: ['#6366f1', '#a855f7'], height: 36, ...overrides };
}

function assertComplete(result: { content: string; contentType: string }): void {
...
describe('createTerminalTyping', () => {
  test('returns complete SVG', () => {
    const result = createTerminalTyping(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: '$ npm install' }),
    );
    assertComplete(result);
  });
  ...
  test('contains terminal dots and prompt', () => {
    const result = createTerminalTyping(
      ...
      animationConfig,
      '3s',
      defaultParams({ text: '$ npm install', colors: ['#a855f7'] }),
    );
    expect(result.content).toContain('#ff5f56');
    expect(result.content).toContain('#ffbd2e');
    expect(result.content).toContain('#27c93f');
    expect(result.content).toContain('$');
    expect(result.content).toContain('#a855f7');
  });
  ...
  test('contains typing animation and blinking cursor', () => {
    ...
    expect(result.content).toContain('#f5f5f5');
  });
});

describe('createSkillPills', () => {
  test('returns complete SVG', () => {
    const result = createSkillPills(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'React;Node.js' }),
    );
    assertComplete(result);
  });
  ...
  test('contains each skill text', () => {
    const result = createSkillPills(
      ...
      animationConfig,
      '2s',
      defaultParams({ text: 'React;Node.js;Python', colors: ['#61dafb', '#339933', '#3776ab'] }),
    );
  ...
  test('uses alternating colors and SMIL animate', () => {
    const result = createSkillPills(
      ...
      animationConfig,
      '2s',
      defaultParams({ text: 'A;B', colors: ['#ff0000', '#00ff00'] }),
    );
...
describe('createShimmerBanner', () => {
  test('returns complete SVG', () => {
    const result = createShimmerBanner(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'Banner' }),
    );
    assertComplete(result);
  });
  ...
  test('contains text and shimmer animation', () => {
    const result = createShimmerBanner(
      ...
      animationConfig,
      '2s',
      defaultParams({ text: 'Developed by Claude Code' }),
    );
  ...
  test('has rounded corners and edge highlights', () => {
    ...
    expect(result.content).toContain('rgba(255,255,255,0.3)');
    expect(result.content).toContain('rgba(0,0,0,0.2)');
  });
});

describe('createShimmerText', () => {
  test('returns complete SVG', () => {
    const result = createShimmerText(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'Glow' }),
    );
    assertComplete(result);
  });
  ...
  test('uses base and accent colors', () => {
    const result = createShimmerText(
      ...
      animationConfig,
      '3s',
      defaultParams({ colors: ['#ff0000', '#00ff00'] }),
    );
    expect(result.content).toContain('#ff0000');
    expect(result.content).toContain('#00ff00');
  });
...
describe('createGoldBadge', () => {
  test('returns complete SVG', () => {
    const result = createGoldBadge(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'Premium' }),
    );
    assertComplete(result);
  });
  ...
  test('has depth overlay and edge lines', () => {
    ...
    expect(result.content).toContain('rgba(255,255,255,0.5)');
    expect(result.content).toContain('rgba(0,0,0,0.3)');
  });
});

describe('createSocialBadge', () => {
  test('returns complete SVG', () => {
    const result = createSocialBadge(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'Follow' }),
    );
    assertComplete(result);
  });
...
describe('createRepoCard', () => {
  test('returns complete SVG', () => {
    const result = createRepoCard(
      stops,
      animationConfig,
      animationDuration,
      defaultParams({ text: 'my-repo' }),
    );
    assertComplete(result);
  });
  ...
  test('contains repo name and GitHub icon path', () => {
    const result = createRepoCard(
      ...
      animationConfig,
      '3s',
      defaultParams({ text: 'awesome-lib' }),
    );
    expect(result.content).toContain('awesome-lib');
    expect(result.content).toContain('M12 0C5.37');
  });
  ...
  test('has two panels with distinct colors', () => {
    const result = createRepoCard(
      ...
      animationConfig,
      '3s',
      defaultParams({ colors: ['#24292e', '#f6f8fa'] }),
    );
