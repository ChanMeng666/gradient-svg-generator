...
const { createTurbulenceFilter, createGlowFilter, createBlurFilter } = require('../../core/FilterLibrary');
...
function createTurbulentWavesGradient(stops, animationConfig, animationDuration) {
  const waveFilter = createTurbulenceFilter('waveDistortion', {
    baseFrequency: '0.5',
    numOctaves: 4,
    scale: 30,
    animated: true,
  ...
  return {
    gradientDef: `
      <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <animate attributeName="y1" values="0%;50%;0%" dur="${multiplyDuration(animationDuration, 1.3)}" repeatCount="indefinite" />
      </linearGradient>
      ${waveFilter}`,
    additionalElements: `
...
function createAuroraWavesGradient(stops, animationConfig, animationDuration) {
  const auroraFilter = createGlowFilter('auroraGlow', {
    color: '#ffffff',
    intensity: 12,
    opacity: 1
  });
  ...
  return {
    gradientDef: `
      <linearGradient id="gradient" x1="0%" y1="50%" x2="100%" y2="50%">
        ...
        <animate attributeName="x2" values="20%;120%;20%" dur="${multiplyDuration(animationDuration, 0.5)}" repeatCount="indefinite" />
      </linearGradient>`,
    additionalElements: `
      <g transform="translate(400, 300)">
        <g stroke="url(#gradient)" fill="none" stroke-width="4">
          <path d="M-300 0 Q-250 -30 -200 0 Q-150 30 -100 0 Q-50 -30 0 0 Q50 30 100 0 Q150 -30 200 0 Q250 30 300 0">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 0.3)}" repeatCount="indefinite"
          ...
          <path d="M-300 0 Q-250 -20 -200 0 Q-150 20 -100 0 Q-50 -20 0 0 Q50 20 100 0 Q150 -20 200 0 Q250 20 300 0" opacity="0.7">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 0.4)}" repeatCount="indefinite"
          ...
          <path d="M-300 0 Q-250 -40 -200 0 Q-150 40 -100 0 Q-50 -40 0 0 Q50 40 100 0 Q150 -40 200 0 Q250 40 300 0" opacity="0.5">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 0.6)}" repeatCount="indefinite"
...
function createCryogenicWavesGradient(stops, animationConfig, animationDuration) {
  const cryoFilter = createTurbulenceFilter('cryogenicVapor', {
    baseFrequency: '0.6',
    numOctaves: 3,
    scale: 20,
    animated: true,
  ...
  return {
    gradientDef: `
      <radialGradient id="gradient" cx="50%" cy="80%" r="60%">
        ...
        <animate attributeName="r" values="50%;70%;50%" dur="${multiplyDuration(animationDuration, 1.5)}" repeatCount="indefinite" />
      </radialGradient>
      ${cryoFilter}`,
    additionalElements: `
      <g>
        ...
        <g opacity="0.6">
          <circle cx="300" cy="450" r="15" fill="url(#gradient)" filter="url(#cryogenicVapor)">
            <animate attributeName="cy" values="450;200;450" dur="${multiplyDuration(animationDuration, 0.8)}" repeatCount="indefinite" />
            <animate attributeName="r" values="10;25;10" dur="${multiplyDuration(animationDuration, 0.6)}" repeatCount="indefinite" />
          </circle>
          <circle cx="400" cy="470" r="20" fill="url(#gradient)" filter="url(#cryogenicVapor)">
            <animate attributeName="cy" values="470;150;470" dur="${multiplyDuration(animationDuration, 1.1)}" repeatCount="indefinite" />
            <animate attributeName="r" values="15;30;15" dur="${multiplyDuration(animationDuration, 0.7)}" repeatCount="indefinite" />
          </circle>
          <circle cx="500" cy="460" r="18" fill="url(#gradient)" filter="url(#cryogenicVapor)">
            <animate attributeName="cy" values="460;180;460" dur="${multiplyDuration(animationDuration, 0.9)}" repeatCount="indefinite" />
            <animate attributeName="r" values="12;28;12" dur="${multiplyDuration(animationDuration, 0.8)}" repeatCount="indefinite" />
          </circle>
        </g>
      </g>`
  };
...
function createSolarWavesGradient(stops, animationConfig, animationDuration) {
  const solarFilter = createGlowFilter('solarFlare', {
    color: '#ffffff',
    intensity: 6,
    opacity: 1
  });
  ...
  return {
    gradientDef: `
      <radialGradient id="gradient" cx="20%" cy="20%" r="80%">
        ...
        <animate attributeName="cy" values="20%;40%;20%" dur="${multiplyDuration(animationDuration, 1.2)}" repeatCount="indefinite" />
      </radialGradient>
      ${solarFilter}`,
    additionalElements: `
      <g>
        ...
        <g stroke="url(#gradient)" fill="none" stroke-width="8" opacity="0.8">
          <path d="M160 100 Q300 80 450 120 Q600 160 750 140">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 0.7)}" repeatCount="indefinite"
          ...
          <path d="M140 130 Q280 200 420 180 Q560 160 700 200" stroke-width="6" opacity="0.6">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 0.9)}" repeatCount="indefinite"
          ...
          <path d="M150 70 Q290 40 430 80 Q570 120 710 100" stroke-width="6" opacity="0.6">
            <animate attributeName="d" dur="${multiplyDuration(animationDuration, 1.1)}" repeatCount="indefinite"
