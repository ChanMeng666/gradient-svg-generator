/**
 * Preset filter catalog — the 20+ <filter> definitions that every SVG embeds
 * in its <defs> block via SVGComposer.getAllFilters(). These are the standard
 * effects every generator can reference by ID (e.g. `filter="url(#glitchEffect)"`)
 * without having to emit the filter body themselves.
 *
 * Kept as a single template literal rather than programmatically composed
 * because these are static, well-known filters that never change shape —
 * the only parametric input is `height` for positioning light sources.
 */

/**
 * Get all standard filter definitions.
 *
 * @param {number} [height=120] - SVG height for positioning light sources
